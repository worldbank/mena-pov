...
rwi_sf <- st_set_crs(rwi_sf_temp, 4326) %>% 
  st_transform(st_crs(jor_district))

# Spatially join the point data to the shapefile based on which polygon each point falls within
merged_data <- st_join(rwi_sf, jor_district, left = TRUE)

## Aggregate by district
merged_data <- merged_data %>%
  group_by(NAME_2) %>%
  mutate(mean_rwi = mean(rwi)) %>%
  ungroup()
...
ggplot() +
  geom_sf(data = jor_district, fill = "black", alpha = 0.9, linewidth = 0.1) +
  geom_sf(data = merged_data, aes(color = mean_rwi), size = 0.8, alpha = 2) +
