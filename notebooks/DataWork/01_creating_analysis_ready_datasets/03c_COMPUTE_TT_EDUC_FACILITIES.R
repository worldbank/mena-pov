# ================================================================
# Script Name: 03b_COMPUTE_TT_EDUC_FACILITIES.R
# Purpose: Compute the travel time by car to educational facilties
# Input Dataset:  grid_10km.Rds, mena_educ_facilities.Rds,grid_10km.Rds
...
# Load Data ---------------------------------------------------------------
educ_fac <- readRDS(file.path(final_replication,"mena_educ_facilities.Rds"))
...
output_dir <-paste0("M:/MENA/GEO/OpenStreetMap/temp/educ/", COUNTRY_NAME) # Replace Path 
...
educ_fac_country <- educ_fac %>%
  dplyr::filter(country_name == COUNTRY_NAME) %>%
  dplyr::select(osm_id, geometry) %>%
  st_centroid() %>%
  st_coordinates() %>%
  as.data.frame() %>%
  cbind(educ_fac$osm_id[educ_fac$country_name == COUNTRY_NAME]) %>%
...
#Note: For Algeria, need to break it into two grids to process the code -- more computation
#For Algeria 
#grid_sf_country_1 <- grid_sf_country[1:15000,]
#grid_sf_country_2 <- grid_sf_country[15001:33863,]
...
destination_chunks <- split_into_chunks(educ_fac_country, chunk_size)
...
output_files <- parLapply(cl, source_indices, function(index) {
  process_source(index, grid_sf_country, destination_chunks, output_dir)
}) #Change grid name for Algeria and Iran
...
# Combine CSVs ------------------------------------------------------------
folder_path <- "M:/MENA/GEO/OpenStreetMap/temp/educ/Islamic Republic of Iran"
...
# Export ------------------------------------------------------------------
output_file_name <- file.path(final_replication,"OSM", "final", "educ", paste0("final_output_", COUNTRY_NAME, ".csv")) #INSERT COUNTRY NAME
