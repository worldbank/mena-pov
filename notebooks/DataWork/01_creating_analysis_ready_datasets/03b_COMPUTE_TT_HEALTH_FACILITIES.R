# ================================================================
# Script Name: 03b_COMPUTE_TT_HEALTH_FACILITIES.R
# Purpose: Compute the travel time by car
# Input Dataset: "mena_health_facilities.Rds, grid_10km.Rds, MENA_ADM0.shp
...
#Note: Update list once the code is completed for a specific country. Repeat code for each country

# Countries Completed: # Countries Completed: Bahrain, Djibouti, Arab Republic of Egypt, Republic of Yemen,Saudi Arabia, Oman, United Arab Emirates,
# Qatar,Kuwait,Israel,West Bank and Gaza,Lebanon, Morocco, Libya, Iraq,Tunisia, Jordan, Syrian Arab Republic, Algeria,
#Islamic Republic of Iran


# Load Data ---------------------------------------------------------------
health_fac <- readRDS(file.path(final_replication,"mena_health_facilities.Rds"))
...
output_dir <- paste0("M:/MENA/GEO/OpenStreetMap/temp/health/", COUNTRY_NAME)  # Create your own folder path
...
health_fac_country <- health_fac %>%
  dplyr::filter(country_name == COUNTRY_NAME) %>%
  dplyr::select(osm_id, geometry) %>%
  st_centroid() %>%
  st_coordinates() %>%
  as.data.frame() %>%
  cbind(health_fac$osm_id[health_fac$country_name == COUNTRY_NAME]) %>%
...
grid_sf_country <- grid_sf_country[grid_sf_country$grid_id %in% non_matching_entries$grid_id, ]

# Note: For Algeria and Iran split the code into two grids
#For Algeria 
...
destination_chunks <- split_into_chunks(health_fac_country, chunk_size) # Split destinations into chunks
...
# Combine CSVs ------------------------------------------------------------
# #For Algeria and Iran pulled all the CSVs after breaking the grid into two
folder_path <- "M:/MENA/GEO/OpenStreetMap/temp/health/Islamic Republic of Iran"
...
# Export ------------------------------------------------------------------
output_file_name <- file.path(raw_replication,"OSM", "intermediate", "health", paste0("final_output_", COUNTRY_NAME, ".csv")) #INSERT COUNTRY NAME 
